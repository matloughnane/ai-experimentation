...
export function createCommunityTools(config: CommunityConfig) {
  ...
  const communityCategoriesTools = createTool({
    ...
    execute: async () => {
      console.log(`\n🏘️ [${name}] COMMUNITY CATEGORIES REQUEST`);
      console.log(`${apiBase}/categories`);

      try {
        const response = await fetch(`${apiBase}/categories`);

        console.log(`🏘️ [${name}] COMMUNITY CATEGORIES RESPONSE STATUS:`, response.status, response.statusText);
  ...
  const communityPagesByCategoryTool = createTool({
    ...
    execute: async ({ categoryId, categoryName, pageName }) => {
      ...
      console.log(`\n📄 [${name}] COMMUNITY PAGES REQUEST:`, { categoryId, categoryName, pageName });

      try {
        const response = await fetch(`${apiBase}/page/category/${categoryId}`);

        console.log(`📄 [${name}] COMMUNITY PAGES RESPONSE STATUS:`, response.status, response.statusText);
  ...
  const communityEventsTool = createTool({
    ...
    execute: async ({ startTimestamp, endTimestamp }) => {
      console.log(`\n📅 [${name}] COMMUNITY EVENTS REQUEST:`, { startTimestamp, endTimestamp });

      try {
        const response = await fetch(`${apiBase}/events/${startTimestamp}/${endTimestamp}`);

        console.log(`📅 [${name}] COMMUNITY EVENTS RESPONSE STATUS:`, response.status, response.statusText);
